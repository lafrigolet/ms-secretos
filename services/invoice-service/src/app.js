...
await app.register(swaggerPlugin, {
  openapi: {
    info: { title: 'Secretos del Agua — Invoice Service', description: 'Descarga de facturas desde SAP (HU-20)', version: '1.0.0' },
    tags: [{ name: 'invoices', description: 'Facturas' }],
...
app.get('/health', async () => ({ status: 'ok', service: 'sda-invoice-service', uptime: process.uptime(), timestamp: new Date().toISOString() }))

const PORT = Number(process.env.PORT ?? 3008)
try {
  await app.listen({ port: PORT, host: process.env.HOST ?? '0.0.0.0' })
  app.log.info(`Invoice Service en puerto ${PORT}`)
